...
async function renderPos(): Promise<void> {
  ...
  shell(`${title("Registrar venta", "Selecciona el producto, los sabores y confirma el cobro.")}<div class="pos"><section class="stack"><div class="panel"><h2 class="section-title">1. Presentación y tamaño</h2><div class="grid product-grid">${productButtons || "<div class=empty>No hay productos activos.</div>"}</div></div><div class="panel"><h2 class="section-title">2. Sabores${chosenProduct ? ` · elige ${chosenProduct.max_flavors}` : ""}</h2><div class="grid flavors">${flavorButtons}</div></div><div class="panel quantity-panel"><h2 class="section-title">Cantidad</h2><div class="quantity-control"><button class="quantity-button" data-quantity-delta="-1" aria-label="Disminuir cantidad">−</button><output id="quantity">1</output><button class="quantity-button" data-quantity-delta="1" aria-label="Aumentar cantidad">+</button></div><button class="primary pos-action" id="add-cart" ${!chosenProduct || chosenFlavors.length !== chosenProduct.max_flavors ? "disabled" : ""}>Agregar al carrito</button></div></section><aside class="panel cart"><h2 class="section-title">Venta actual</h2><div class="cart-items">${cartItems}</div><div class="total"><span>Total</span><span>${money(total)}</span></div><section class="payment-section"><h2 class="section-title">Método de pago</h2><div class="payment-options">${paymentButtons}</div></section>${selectedPayment === "other" ? `<label class="field payment-comment">Comentario para Otro<input id="payment-comment" maxlength="300" placeholder="Describe el método de pago"></label>` : ""}<button class="primary checkout-action" id="confirm-sale" ${!cart.length ? "disabled" : ""}>Confirmar venta</button></aside></div>`, "pos");
  ...
  document.querySelector<HTMLButtonElement>("#add-cart")?.addEventListener("click", () => { const quantity = Number(document.querySelector<HTMLOutputElement>("#quantity")!.value); if (chosenProduct && quantity > 0) { cart.push({ product: chosenProduct, flavors: chosenFlavors, quantity }); chosenProduct = null; chosenFlavors = []; renderPos().catch(showError); } });
