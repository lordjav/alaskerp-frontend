...
function shell(content: string, page: Page): void {
  ...
  app.innerHTML = `<div class="shell"><aside class="side"><div class="brand">alaska<small>Helados · ERP</small></div><nav class="nav">${nav}</nav><footer class="side-user"><div class="user-card"><span class="user-avatar">${(username()[0] ?? "?").toUpperCase()}</span><span class="user-name" title="${username()}">${capitalize(username())}</span></div><button class="logout" data-logout><svg class="logout-icon" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Cerrar sesión</button></footer></aside><main class="page ${page === "pos" ? "pos-page" : ""}">${content}</main></div>`;
