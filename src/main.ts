...
function shell(content: string, page: Page): void {
  ...
  app.innerHTML = `<div class="shell"><aside class="side"><div class="brand">alaska<small>Helados · ERP</small></div><nav class="nav">${nav}</nav><footer><strong>${roles().join(" · ")}</strong><br><button data-logout>Cerrar sesión</button></footer></aside><main class="page">${content}</main></div>`;
...
async function renderPos(): Promise<void> {
  ...
  const flavorButtons = chosenProduct ? [...flavors.map((f) => f.name), "Otro"].map((flavor) => `<button class="choice ${chosenFlavors.includes(flavor) ? "selected" : ""}" data-flavor="${flavor}">${flavor}</button>`).join("") : `<div class="empty">Selecciona primero una presentación y tamaño.</div>`;
  const cartItems = cart.length ? cart.map((item, index) => `<div class="cart-item"><div><strong>${item.product.presentation} · ${item.product.size}</strong><br><small>${item.flavors.join(", ")} · Cantidad ${item.quantity}</small></div><div><strong>${money(item.product.price * item.quantity)}</strong><br><button class="outline" data-remove="${index}">Quitar</button></div></div>`).join("") : `<div class="empty">Tu carrito está vacío.</div>`;
  const total = cart.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
  shell(`${title("Registrar venta", "Selecciona el producto, los sabores y confirma el cobro.")}<div class="pos"><section class="stack"><div class="panel"><h2 class="section-title">1. Presentación y tamaño</h2><div class="grid">${productButtons || "<div class=empty>No hay productos activos.</div>"}</div></div><div class="panel"><h2 class="section-title">2. Sabores${chosenProduct ? ` · elige ${chosenProduct.max_flavors}` : ""}</h2><div class="grid flavors">${flavorButtons}</div></div><div class="panel"><div class="form-grid"><label class="field">Cantidad<input id="quantity" type="number" min="1" max="50" value="1"></label></div><div class="actions" style="margin-top:14px"><button class="primary" id="add-cart" ${!chosenProduct || chosenFlavors.length !== chosenProduct.max_flavors ? "disabled" : ""}>Agregar al carrito</button></div></div></section><aside class="panel cart"><h2 class="section-title">Venta actual</h2><div class="cart-items">${cartItems}</div><div class="total"><span>Total</span><span>${money(total)}</span></div><label class="field">Método de pago<select id="payment"><option value="cash">Efectivo</option><option value="nequi">Nequi</option><option value="bancolombia">Bancolombia</option><option value="credit_card">Tarjeta de crédito</option><option value="other">Otro</option></select></label><label class="field" style="margin-top:12px">Comentario para Otro<input id="payment-comment" maxlength="300" placeholder="Opcional excepto para Otro"></label><button class="primary" id="confirm-sale" style="margin-top:16px" ${!cart.length ? "disabled" : ""}>Confirmar venta</button></aside></div>`, "pos");
  document.querySelectorAll<HTMLButtonElement>("[data-product]").forEach((button) => button.onclick = () => { chosenProduct = products.find((p) => p.id === button.dataset.product)!; chosenFlavors = []; renderPos().catch(showError); });
  document.querySelectorAll<HTMLButtonElement>("[data-flavor]").forEach((button) => button.onclick = () => { const flavor = button.dataset.flavor!; chosenFlavors = chosenFlavors.includes(flavor) ? chosenFlavors.filter((item) => item !== flavor) : chosenFlavors.length < (chosenProduct?.max_flavors ?? 0) ? [...chosenFlavors, flavor] : chosenFlavors; renderPos().catch(showError); });
  ...
  document.querySelector<HTMLButtonElement>("#add-cart")?.addEventListener("click", () => { const quantity = Number(document.querySelector<HTMLInputElement>("#quantity")!.value); if (chosenProduct && quantity > 0) { cart.push({ product: chosenProduct, flavors: chosenFlavors, quantity }); chosenProduct = null; chosenFlavors = []; renderPos().catch(showError); } });
...
async function confirmSale(): Promise<void> {
  const payment = document.querySelector<HTMLSelectElement>("#payment")!.value;
